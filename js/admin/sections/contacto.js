/* ═══════════════════════════════════════════════════════════
   SEAMOS GENIOS — CMS Section: CONTACTO
   ═══════════════════════════════════════════════════════════ */

window.SGSection_contacto = (() => {

    function render(container) {
        const data = SGStore.get();
        // Here you would normally extract the section data: const secData = data.contacto || {};
                ...
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-white flex items-center gap-3">
                            <i data-lucide="edit-3" class="w-6 h-6 text-sg-cyan"></i> Editor de ${capitalizeFirstLetter('contacto')}
                        </h2>
                        <p class="text-sm text-gray-400">Configura el contenido y propiedades de esta sección.</p>
                    </div>
                    <button id="save-contacto-btn" class="bg-gradient-to-r from-sg-cyan to-blue-500 hover:from-cyan-400 text-white font-semibold py-2 px-5 rounded-lg flex items-center gap-2 shadow-[0_0_15px_rgba(6,182,212,0.4)] transition-all">
        ...
        // Bind events
        const saveBtn = container.querySelector(`#save-contacto-btn`);
        if (saveBtn) {
